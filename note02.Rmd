...
```{r}
covid19.ts <- ts(covid19[, 2:10], frequency = 365, start = c(2020, 04, 09))
...
dataset <- ls()
target <-
  dataset %in% c("students", "employees", "covid19") |
    stringr::str_detect(dataset, "[.]ts$") |
    stringr::str_detect(dataset, "[.]xts$") |
    stringr::str_detect(dataset, "[.]tsibble$")
dataset[target]
```
